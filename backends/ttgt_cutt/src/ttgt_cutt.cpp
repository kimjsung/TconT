  ...
  T* d_A, T* d_A_trans, T* d_B, T* d_B_trans, T* d_C, T* d_C_trans)
{
  ...
  constexpr size_t CUBLAS_WORKSPACE_SIZE = 32ULL * 1024 * 1024;
  check_cuda_or_throw(cudaMalloc(&runtime.cublas_workspace, CUBLAS_WORKSPACE_SIZE), "cudaMalloc(cublas_workspace)");
  runtime.cublas_workspace_size = CUBLAS_WORKSPACE_SIZE;
  check_cublas_or_throw(cublasSetWorkspace(runtime.cublas_handle, runtime.cublas_workspace, CUBLAS_WORKSPACE_SIZE), "cublasSetWorkspace");
...
void ttgt_cuTT_runtime_destroy(ttgt_cuTT_runtime& runtime)
{
  ...
  if (runtime.cublas_workspace != nullptr) {
    cudaFree(runtime.cublas_workspace);
    runtime.cublas_workspace      = nullptr;
    runtime.cublas_workspace_size = 0;
  }
