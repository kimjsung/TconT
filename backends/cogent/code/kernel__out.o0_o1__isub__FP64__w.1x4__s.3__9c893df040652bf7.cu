...
#define plane_t2 			(TILE_C * TILE_A1)
...
int shm_v2_offset, int shm_t2_offset, int iter_c)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_src_v2 = v2_c + iter_c + (blk_idx_b * TILE_B + v2_b2 * TILE_B1 + v2_b1) * size_c;

		int sm_dst_v2 = shm_v2_offset + (v2_b2 * (TILE_B1 * TILE_C)) + (v2_c ^ (((v2_b1 >> 1) & 1) << 2)) + (v2_b1 << 3);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int t2_c = rem_t2 / TILE_A1;
		int t2_a1 = rem_t2 - t2_c * TILE_A1;

		int sm_src_t2 = blk_idx_a * TILE_A + t2_a2 * TILE_A1 + t2_a1 + (t2_c + iter_c) * size_a;

		int sm_dst_t2 = shm_t2_offset + (t2_a2 * (TILE_C * TILE_A1)) + (t2_a1 ^ (((t2_c >> 1) & 1) << 2)) + (t2_c << 3);
...
int blk_idx_a, int blk_idx_b, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_v2_offset, int shm_t2_offset, int iter_c, int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		int v2_b2 = idx / plane_v2;
...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	const int ld_stride_b = TILE_C * TILE_A1;
...
__device__ __forceinline__ void scatter_store_tail(double *__restrict__ dev_t3, int m_base, int n_base, int rng_b, int rng_a, int intra_stride,
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	const int wmiter = TILE_B2 / 4;
	const int wniter = TILE_A2 / 1;
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', '', 't2', ['a', 'c']], [16, 'STR_SD2_V2_H7', '', 'v2', ['c', 'b']], '-']
