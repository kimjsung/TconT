...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		{
			int v2_offset = shm_v2_offset + ((wrow ^v2_fragment_offset) ^ iter_v2) + v2_lane_offset + (ll << 3);
			...
			for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
			{
				...
				const int out_idx0 = ((iter_v2) * wniter) + iter_t2;
				const int out_idx1 = ((iter_v2 + 1) * wniter) + iter_t2;
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	const int wmiter = TILE_D / 1;
	const int wniter = TILE_B / 2;
