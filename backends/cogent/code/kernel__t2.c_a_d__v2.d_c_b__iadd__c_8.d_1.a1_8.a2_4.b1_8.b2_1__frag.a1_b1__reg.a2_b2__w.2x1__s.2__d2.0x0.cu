...
int shm_v2_offset, int shm_t2_offset, int iter_c, int iter_d)
{
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		...
		int sm_dst_t2 = shm_t2_offset + (t2_a2 * (TILE_A1 * TILE_C)) + ((t2_a1 ^ ((t2_c >> 2) << 1)) << 2) + (t2_c & 3) + ((t2_c >> 2) << 5);
...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
		{
			...
			{
				int t2_offset = shm_t2_offset + (wcol + iter_t2) * ld_stride_b + (ll << 3);
