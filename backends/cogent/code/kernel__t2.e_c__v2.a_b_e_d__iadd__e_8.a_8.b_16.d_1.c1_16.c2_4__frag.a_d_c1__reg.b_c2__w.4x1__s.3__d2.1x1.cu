...
#define plane_t2 			(TILE_C1 * TILE_E)
#define t2_elements 		(plane_t2 * TILE_C2)

#define plane_v2 			(TILE_E * TILE_A)
#define v2_elements 		(plane_v2 * TILE_B)
...
int shm_t2_offset, int shm_v2_offset, int iter_e)
{
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		int t2_c2 = idx / plane_t2;
		int rem_t2 = idx - t2_c2 * plane_t2;
		int t2_c1 = rem_t2 / TILE_E;
		int t2_e = rem_t2 - t2_c1 * TILE_E;

		int sm_src_t2 = t2_e + iter_e + (blk_idx_c * TILE_C + t2_c2 * TILE_C1 + t2_c1) * size_e;

		int sm_dst_t2 = shm_t2_offset + (t2_c2 * (TILE_C1 * TILE_E)) + (t2_e ^ (((t2_c1 >> 1) & 1) << 2)) + (t2_c1 << 3);
	...
	{
		int v2_b = idx / plane_v2;
		int rem_v2 = idx - v2_b * plane_v2;
		int v2_e = rem_v2 / TILE_A;
		int v2_a = rem_v2 - v2_e * TILE_A;

		int sm_src_v2 = blk_idx_a * TILE_A + v2_a + (blk_idx_b * TILE_B + v2_b + (v2_e + iter_e + (blk_idx_d * TILE_D) * size_e) * size_b) * size_a;

		int sm_dst_v2 = shm_v2_offset + (v2_b * (TILE_E * TILE_A)) + (v2_a ^ (((v2_e >> 1) & 1) << 2)) + (v2_e << 3);
...
int blk_idx_a, int blk_idx_b, int blk_idx_c, int blk_idx_d, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_t2_offset, int shm_v2_offset, int iter_e, int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		int t2_c2 = idx / plane_t2;
...
int rng_c, int rng_b)
{
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		if((t2_c2 * TILE_C1 + t2_c1 < rng_c) && (t2_e < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_t2[sm_dst_t2]), reinterpret_cast<const double2*>(&dev_t2[sm_src_t2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
	...
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_e < TILE_UNIT - internal_upperbound) && (v2_b < rng_b))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int internal_upperbound)
{
	...
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_a < rng_a) && (v2_e < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int rng_b)
{
	...
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_a < rng_a) && (v2_e < TILE_UNIT - internal_upperbound) && (v2_b < rng_b))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_C1 * TILE_E;
	const int ld_stride_b = TILE_E * TILE_A;

	nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 8, 8, 4, double, nvcuda::wmma::row_major> t2_frag;
	...
	const int v2_fragment_offset = ((((lane & 3) << 1) ^ ((lane >> 1) & 1)) ^ (lane >> 4));
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wmiter; iter_t2++)
		{
			...
			{
				int t2_offset = shm_t2_offset + ((wrow + iter_t2) * ld_stride_a) + (cnt_t2 << 6) + ((t2_fragment_offset ^ (ll >> 2)) << 2) + t2_lane_offset;
				t2_frag.x[0] = sm_t2[t2_offset];

				#pragma unroll
				for(int iter_v2 = 0; iter_v2 < wniter; iter_v2++)
				{
					int v2_offset = shm_v2_offset + ((wcol + iter_v2) * ld_stride_b) + (v2_fragment_offset << 2) + v2_lane_offset + (ll << 3);
					const int out_idx = ((iter_t2 * wniter) + iter_v2) * t2_frag_cnt + cnt_t2;
					v2_frag.x[0] = sm_v2[v2_offset];
					nvcuda::wmma::mma_sync(t3_frag[out_idx], t2_frag, v2_frag, t3_frag[out_idx]);
				}
			}
		}
	}
...
__device__ __forceinline__ void scatter_store_tail0(double *dev_t3, int m_base, int g_n_iter, int rng_c, int rng_b, int intra_stride,
...
__device__ __forceinline__ void scatter_store_tail2(double *dev_t3, int m_base, int g_n_iter, int n_base, int rng_c, int rng_b, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
...
int size_internal, int t2_tile_size, int v2_tile_size)
{
	...
	// Fragment Allocation
	const int t2_frag_cnt = TILE_C1 >> 3;
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', '', 't2', ['e', 'c']], [16, 'STR_SD2_V2_H7', 'x', 'v2', ['a', 'b', 'e', 'd']], '+=']
...
int size_internal, int t2_tile_size, int v2_tile_size)
{
	...
	{
		int base = t3_base_thread + iter_t2 * inter_m_stride;
		int g_m_iter = (wrow + iter_t2) * TILE_C1;
		#pragma unroll
		for(int iter_v2 = 0; iter_v2 < wniter; iter_v2++)
		{
			int base_iter = base + iter_v2 * inter_n_stride;
