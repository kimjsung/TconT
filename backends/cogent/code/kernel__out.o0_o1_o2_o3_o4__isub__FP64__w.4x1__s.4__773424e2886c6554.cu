...
#define plane_v2 			(TILE_F * TILE_B1)
...
int shm_v2_offset, int shm_t2_offset, int iter_f)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		int v2_f = rem_v2 / TILE_B1;
		int v2_b1 = rem_v2 - v2_f * TILE_B1;

		int sm_src_v2 = blk_idx_b * TILE_B + v2_b2 * TILE_B1 + v2_b1 + (v2_f + iter_f) * size_b;

		int sm_dst_v2 = shm_v2_offset + (v2_b2 * (TILE_F * TILE_B1)) + (v2_b1 ^ (((v2_f >> 0) & 3) << 2)) + (v2_f << 4);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_src_t2 = blk_idx_e * TILE_E + t2_e + (t2_f + iter_f + (blk_idx_c * TILE_C + (blk_idx_a * TILE_A + t2_a + (blk_idx_d * TILE_D) * size_a) * size_c) * size_f) * size_e;
...
int rng_b, int rng_e)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_b2 * TILE_B1 + v2_b1 < rng_b) && (v2_f < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_F * TILE_B1;
...
__device__ __forceinline__ void scatter_store_tail0(double *__restrict__ dev_t3, int m_base, int g_n_iter, int rng_b, int rng_e, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	...
	if((m_base + id_m) < rng_b && g_n_iter < rng_e)
	{
...
__device__ __forceinline__ void scatter_store_tail2(double *__restrict__ dev_t3, int m_base, int g_n_iter, int rng_b, int rng_e, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
	...
	int id_m = frag_m - div4;
	int id_n = frag_n + div4;
	if((m_base + id_m) < rng_b && g_n_iter < rng_e && (id_n) < rng_a)
	{
		dev_t3[id_m * intra_stride + id_n] = (div4 ? y : x);
	}
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', 'x', 't2', ['e', 'f', 'c', 'a', 'd']], [16, 'STR_SD2_V2_H7', '', 'v2', ['b', 'f']], '-']
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				int dst = base_v2;
				const int frag_idx = frag_tmp2;
				bool full = ((m_base + 8 <= rng_b) && (g_n_iter < rng_e) && (((uintptr_t)(&dev_t3[dst]) & 0x1F) == 0) && ((intra_stride & 1) == 0));
				if(full)
				{
					nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
				}
				...
				{
					scatter_store_tail0(&dev_t3[dst], m_base, g_n_iter, rng_b, rng_e, intra_stride, t3_frag[frag_idx]);
				}
			}
		}
	}
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				{
					scatter_store_tail2(&dev_t3[dst], m_base, g_n_iter, rng_b, rng_e, rng_a, intra_stride, t3_frag[frag_idx]);
				}
			}
		}
	}
