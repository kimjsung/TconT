...
int shm_v2_offset, int shm_t2_offset, int iter_e, int iter_f)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		int v2_c = idx / plane_v2;
		int rem_v2 = idx - v2_c * plane_v2;
		int v2_e = rem_v2 / TILE_D;
		int v2_d = rem_v2 - v2_e * TILE_D;

		int sm_src_v2 = blk_idx_d * TILE_D + v2_d + (iter_f + (blk_idx_c * TILE_C + v2_c + (v2_e + iter_e) * size_c) * size_f) * size_d;

		int sm_dst_v2 = shm_v2_offset + (v2_c * ((TILE_E * TILE_D) + 4)) + (v2_d ^ (((v2_e >> 1) & 1) << 1)) + (v2_e << 3);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_src_t2 = blk_idx_a * TILE_A + t2_a + (t2_e + iter_e + (blk_idx_b * TILE_B + t2_b + (iter_f) * size_b) * size_e) * size_a;

		int sm_dst_t2 = shm_t2_offset + (t2_b * (TILE_E * TILE_A)) + (t2_a ^ (((t2_e >> 1) & 1) << 2)) + (t2_e << 3);
...
int blk_idx_a, int blk_idx_b, int blk_idx_c, int blk_idx_d, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_v2_offset, int shm_t2_offset, int iter_e, int iter_f, int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		int v2_c = idx / plane_v2;
...
int rng_d, int rng_b)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_e < TILE_UNIT - internal_upperbound) && (v2_d < rng_d))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_c < rng_c) && (v2_e < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int rng_d, int rng_b)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_c < rng_c) && (v2_e < TILE_UNIT - internal_upperbound) && (v2_d < rng_d))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_E * TILE_D + 4;
	const int ld_stride_b = TILE_E * TILE_A;

	nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 8, 8, 4, double, nvcuda::wmma::row_major> v2_frag[2];
	nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 8, 8, 4, double, nvcuda::wmma::row_major> t2_frag;

	const int lane = threadIdx.x & 31;
	const int v2_lane_offset = ((lane >> 2) * ld_stride_a);
	const int v2_fragment_offset = ((lane & 3) << 3) + (((lane >> 1) & 1) << 1);
	...
	{
		#pragma unroll
		for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2 += 2)
		{
			#pragma unroll
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				int v2_offset = shm_v2_offset + ((wrow ^ v2_fragment_offset) ^ iter_v2) + (cnt_v2 * (ld_stride_a << 3)) + v2_lane_offset + (ll << 3);
				double2 tmp = reinterpret_cast<double2*>(&sm_v2[v2_offset])[0];
				v2_frag[0].x[0] = tmp.x;
				v2_frag[1].x[0] = tmp.y;
				#pragma unroll
				for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
				{
					...
					const int out_idx1 = (((iter_v2 + 1) * wniter) + iter_t2) * v2_frag_cnt + cnt_v2;
					t2_frag.x[0] = sm_t2[t2_offset];
					nvcuda::wmma::mma_sync(t3_frag[out_idx0], v2_frag[0], t2_frag, t3_frag[out_idx0]);
					nvcuda::wmma::mma_sync(t3_frag[out_idx1], v2_frag[1], t2_frag, t3_frag[out_idx1]);
				}
			}
		}
	}
...
__device__ __forceinline__ void scatter_store_tail0(double *dev_t3, int g_m_iter, int g_n_iter, int rng_d, int rng_b, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	...
	if(g_m_iter < rng_d && g_n_iter < rng_b)
	{
...
__device__ __forceinline__ void scatter_store_tail1(double *dev_t3, int m_base, int n_base, int rng_c, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
...
__device__ __forceinline__ void scatter_store_tail2(double *dev_t3, int g_m_iter, int m_base, int g_n_iter, int n_base, int rng_d, int rng_c, int rng_b, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
	...
	int id_m = frag_m - div4;
	int id_n = frag_n + div4;
	if(g_m_iter < rng_d && (m_base + id_m) < rng_c && g_n_iter < rng_b && (n_base + id_n) < rng_a)
	{
		dev_t3[id_m * intra_stride + id_n] = (div4 ? y : x);
	}
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	const int wmiter = TILE_D / 4;
	const int wniter = TILE_B / 1;
	const int wrow = ((threadIdx.x >> 5) / 1) * wmiter;
	const int wcol = ((threadIdx.x >> 5) % 1) * wniter;

	// Fragment Allocation
	const int v2_frag_cnt = TILE_C >> 3;
	...
	for(int iter_f = 0; iter_f < size_f; iter_f += TILE_F)
	{
		for(int l = -((int)(PIPELINE_STAGES - 1) * TILE_UNIT); l < size_internal; l += TILE_UNIT)
		{
			...
			if(load_idx < size_internal)
			{
				...
				load_from_GM_1(dev_v2, dev_t2, sm_v2, sm_t2,
						...
						pipeline, thread,
						next_shm_offset * v2_tile_size, next_shm_offset * t2_tile_size, load_idx, iter_f);
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++, frag_idx++)
				{
					int n_base = (cnt_t2 << 3);
					int dst = base_v2 + (cnt_t2 << 3);
					bool full = ((g_m_iter < rng_d) && (g_n_iter < rng_b) && (((uintptr_t)dst & 0x1F) == 0) && ((intra_stride & 1) == 0));
					if(full)
					{
						nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
					}
					...
					{
						scatter_store_tail0(&dev_t3[dst], g_m_iter, g_n_iter, rng_d, rng_b, intra_stride, t3_frag[frag_idx]);
					}
				}
			}
		}
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++, frag_idx++)
				{
					int n_base = (cnt_t2 << 3);
					int dst = base_v2 + (cnt_t2 << 3);
					bool full = ((g_m_iter < rng_d) && (m_base + 8 <= rng_c) && (g_n_iter < rng_b) && (n_base + 8 <= rng_a) && (((uintptr_t)dst & 0x1F) == 0) && ((intra_stride & 1) == 0));
					if(full)
					{
						nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
					}
					...
					{
						scatter_store_tail2(&dev_t3[dst], g_m_iter, m_base, g_n_iter, n_base, rng_d, rng_c, rng_b, rng_a, intra_stride, t3_frag[frag_idx]);
					}
				}
			}
		}
