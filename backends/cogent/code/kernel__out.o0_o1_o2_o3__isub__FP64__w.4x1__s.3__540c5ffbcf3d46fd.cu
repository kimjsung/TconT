...
#define plane_v2 			(TILE_C1 * TILE_E)
...
int shm_v2_offset, int shm_t2_offset, int iter_e)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		int v2_c1 = rem_v2 / TILE_E;
		int v2_e = rem_v2 - v2_c1 * TILE_E;

		int sm_src_v2 = v2_e + iter_e + (blk_idx_c * TILE_C + v2_c2 * TILE_C1 + v2_c1) * size_e;

		int sm_dst_v2 = shm_v2_offset + (v2_c2 * (TILE_C1 * TILE_E)) + (v2_e ^ (((v2_c1 >> 1) & 1) << 2)) + (v2_c1 << 3);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_src_t2 = blk_idx_d * TILE_D + t2_d + (blk_idx_b * TILE_B + (t2_e + iter_e + (blk_idx_a * TILE_A + t2_a) * size_e) * size_b) * size_d;

		int sm_dst_t2 = shm_t2_offset + (t2_a * ((TILE_E * TILE_D) + 8)) + (t2_d ^ (((t2_e >> 0) & 3) << 1)) + (t2_e << 4);
...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	const int ld_stride_b = TILE_E * TILE_D + 8;
...
__device__ __forceinline__ void scatter_store_tail0(double *__restrict__ dev_t3, int m_base, int g_n_iter, int rng_c, int rng_d, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	...
	if((m_base + id_m) < rng_c && g_n_iter < rng_d)
	{
...
__device__ __forceinline__ void scatter_store_tail2(double *__restrict__ dev_t3, int m_base, int g_n_iter, int rng_c, int rng_d, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
	...
	int id_m = frag_m - div4;
	int id_n = frag_n + div4;
	if((m_base + id_m) < rng_c && g_n_iter < rng_d && (id_n) < rng_a)
	{
		dev_t3[id_m * intra_stride + id_n] = (div4 ? y : x);
	}
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'b', 'e', 'a']], [16, 'STR_SD2_V2_H7', '', 'v2', ['e', 'c']], '-']
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				int dst = base_v2;
				const int frag_idx = frag_tmp2;
				bool full = ((m_base + 8 <= rng_c) && (g_n_iter < rng_d) && (((uintptr_t)(&dev_t3[dst]) & 0x1F) == 0) && ((intra_stride & 1) == 0));
				if(full)
				{
					nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
				}
				...
				{
					scatter_store_tail0(&dev_t3[dst], m_base, g_n_iter, rng_c, rng_d, intra_stride, t3_frag[frag_idx]);
				}
			}
		}
	}
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				{
					scatter_store_tail2(&dev_t3[dst], m_base, g_n_iter, rng_c, rng_d, rng_a, intra_stride, t3_frag[frag_idx]);
				}
			}
		}
	}
