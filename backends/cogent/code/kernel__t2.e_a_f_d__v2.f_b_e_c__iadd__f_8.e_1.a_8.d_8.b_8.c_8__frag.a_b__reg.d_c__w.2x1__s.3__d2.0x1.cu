...
#define TILE_UNIT 	(TILE_E * TILE_F)

#define plane_v2 			(TILE_B * TILE_F)
#define v2_elements 		(plane_v2 * TILE_C)

#define plane_t2 			(TILE_F * TILE_A)
#define t2_elements 		(plane_t2 * TILE_D)
...
int shm_v2_offset, int shm_t2_offset, int iter_f, int iter_e)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		int v2_c = idx / plane_v2;
		int rem_v2 = idx - v2_c * plane_v2;
		int v2_b = rem_v2 / TILE_F;
		int v2_f = rem_v2 - v2_b * TILE_F;

		int sm_src_v2 = v2_f + iter_f + (blk_idx_b * TILE_B + v2_b + (iter_e + (blk_idx_c * TILE_C + v2_c) * size_e) * size_b) * size_f;

		int sm_dst_v2 = shm_v2_offset + (v2_c * (TILE_B * TILE_F)) + (v2_f ^ (((v2_b >> 1) & 1) << 2)) + (v2_b << 3);
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		int t2_d = idx / plane_t2;
		int rem_t2 = idx - t2_d * plane_t2;
		int t2_f = rem_t2 / TILE_A;
		int t2_a = rem_t2 - t2_f * TILE_A;

		int sm_src_t2 = iter_e + (blk_idx_a * TILE_A + t2_a + (t2_f + iter_f + (blk_idx_d * TILE_D + t2_d) * size_f) * size_a) * size_e;

		int sm_dst_t2 = shm_t2_offset + (t2_d * ((TILE_F * TILE_A) + 4)) + (t2_a << 2) + (t2_f & 3) + ((t2_f >> 2) << 5) + (((t2_a >> 2) + (t2_f >> 2)) * 2);

		cuda::memcpy_async(thread, &sm_t2[sm_dst_t2], &dev_t2[sm_src_t2], cuda::aligned_size_t<8>{8}, pipeline);
	}
}
...
int blk_idx_a, int blk_idx_b, int blk_idx_c, int blk_idx_d, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_v2_offset, int shm_t2_offset, int iter_f, int iter_e, int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		int v2_c = idx / plane_v2;
...
int rng_c, int rng_d)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_f < TILE_UNIT - internal_upperbound) && (v2_c < rng_c))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		...
		if((t2_f < TILE_UNIT - internal_upperbound) && (t2_d < rng_d))
		{
			cuda::memcpy_async(thread, &sm_t2[sm_dst_t2], &dev_t2[sm_src_t2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
...
int internal_upperbound)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_b < rng_b) && (v2_f < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int rng_c, int rng_d)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_b < rng_b) && (v2_f < TILE_UNIT - internal_upperbound) && (v2_c < rng_c))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		...
		if((t2_a < rng_a) && (t2_f < TILE_UNIT - internal_upperbound) && (t2_d < rng_d))
		{
			cuda::memcpy_async(thread, &sm_t2[sm_dst_t2], &dev_t2[sm_src_t2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_B * TILE_F;
	const int ld_stride_b = TILE_A * TILE_F + 4;
	...
	const int t2_fragment_offset = lane + ((lane >> 4) << 1);
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		{
			int v2_offset = shm_v2_offset + ((wrow + iter_v2) * ld_stride_a) + ((v2_fragment_offset ^ (ll >> 2)) << 2) + v2_lane_offset;
			v2_frag.x[0] = sm_v2[v2_offset];

			#pragma unroll
			for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
			{
				int t2_offset = shm_t2_offset + (wcol + iter_t2) * ld_stride_b + (ll << 3) + (ll >> 1);
				const int out_idx = (iter_v2 * wniter) + iter_t2;
				t2_frag.x[0] = sm_t2[t2_offset + t2_fragment_offset];
...
__device__ __forceinline__ void scatter_store_tail0(double *dev_t3, int g_m_iter, int g_n_iter, int rng_c, int rng_d, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	...
	if(g_m_iter < rng_c && g_n_iter < rng_d)
	{
...
__device__ __forceinline__ void scatter_store_tail2(double *dev_t3, int g_m_iter, int m_base, int g_n_iter, int n_base, int rng_c, int rng_b, int rng_d, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
	...
	int id_m = frag_m - div4;
	int id_n = frag_n + div4;
	if(g_m_iter < rng_c && (m_base + id_m) < rng_b && g_n_iter < rng_d && (n_base + id_n) < rng_a)
	{
		dev_t3[id_m * intra_stride + id_n] = (div4 ? y : x);
	}
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	const int wmiter = TILE_C / 1;
	const int wniter = TILE_D / 2;
	const int wrow = ((threadIdx.x >> 5) / 2) * wmiter;
	const int wcol = ((threadIdx.x >> 5) % 2) * wniter;

	// Fragment Allocation
	const int v2_frag_cnt = TILE_B >> 3;
	const int t2_frag_cnt = TILE_A >> 3;

	// Size of fragment stride
	const int inter_m_stride = size_b * size_a;
	const int inter_n_stride = size_c * size_b * size_a;
	...
	for(int iter_e = 0; iter_e < size_e; iter_e += TILE_E)
	{
		for(int l = -((int)(PIPELINE_STAGES - 1) * TILE_UNIT); l < size_internal; l += TILE_UNIT)
		{
			...
			if(load_idx < size_internal)
			{
				...
				load_from_GM_1(dev_v2, dev_t2, sm_v2, sm_t2,
						...
						pipeline, thread,
						next_shm_offset * v2_tile_size, next_shm_offset * t2_tile_size, load_idx, iter_e);
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++, frag_idx++)
				{
					int n_base = (cnt_t2 << 3);
					int dst = base_v2 + (cnt_t2 << 3);
					bool full = ((g_m_iter < rng_c) && (g_n_iter < rng_d) && (((uintptr_t)dst & 0x1F) == 0) && ((intra_stride & 1) == 0));
					if(full)
					{
						nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
					}
					...
					{
						scatter_store_tail0(&dev_t3[dst], g_m_iter, g_n_iter, rng_c, rng_d, intra_stride, t3_frag[frag_idx]);
					}
				}
			}
		}
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++, frag_idx++)
				{
					int n_base = (cnt_t2 << 3);
					int dst = base_v2 + (cnt_t2 << 3);
					bool full = ((g_m_iter < rng_c) && (m_base + 8 <= rng_b) && (g_n_iter < rng_d) && (n_base + 8 <= rng_a) && (((uintptr_t)dst & 0x1F) == 0) && ((intra_stride & 1) == 0));
					if(full)
					{
						nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
					}
					...
					{
						scatter_store_tail2(&dev_t3[dst], g_m_iter, m_base, g_n_iter, n_base, rng_c, rng_b, rng_d, rng_a, intra_stride, t3_frag[frag_idx]);
					}
				}
			}
		}
