...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
		{
			...
			{
				int v2_offset = shm_v2_offset + ((wrow + iter_v2) * ld_stride_a) + ((v2_fragment_offset ^ (cnt_v2 << 1)) << 2) + v2_lane_offset + (ll << 4);
				v2_frag.x[0] = sm_v2[v2_offset];

				#pragma unroll
				for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
				{
					int t2_offset = shm_t2_offset + ((wcol + iter_t2) * ld_stride_b) + ((t2_fragment_offset ^ (ll >> 2)) << 2) + t2_lane_offset;
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// block order (outer -> inner): b -> a -> c
	int tmp_blkIdx;
	const int blk_idx_b = blockIdx.x / (numBlk_a * numBlk_c);
	tmp_blkIdx = blockIdx.x % (numBlk_a * numBlk_c);

	const int blk_idx_a = tmp_blkIdx / (numBlk_c);
	tmp_blkIdx = tmp_blkIdx % (numBlk_c);

	const int blk_idx_c = tmp_blkIdx;
