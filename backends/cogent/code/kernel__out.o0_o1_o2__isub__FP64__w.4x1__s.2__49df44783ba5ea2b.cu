...
int shm_v2_offset, int shm_t2_offset, int iter_d)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_dst_v2 = shm_v2_offset + (v2_b2 * (TILE_D * TILE_B1)) + (v2_b1 ^ (((v2_d >> 0) & 3) << 2)) + (v2_d << 4);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int t2_a = rem_t2 / TILE_D;
		int t2_d = rem_t2 - t2_a * TILE_D;

		int sm_src_t2 = t2_d + iter_d + (blk_idx_c * TILE_C + t2_c + (blk_idx_a * TILE_A + t2_a) * size_c) * size_d;

		int sm_dst_t2 = shm_t2_offset + (t2_c * (TILE_A * TILE_D)) + (t2_d ^ (((t2_a >> 0) & 3) << 2)) + (t2_a << 4);
...
const int shm_v2_offset, const int shm_t2_offset)
{
	...
	const int ld_stride_b = TILE_A * TILE_D;
	...
	const int t2_fragment_offset = (((lane >> 2) << 2) ^ ((lane >> 2) & 3));
...
__device__ __forceinline__ void scatter_store_tail2(double *__restrict__ dev_t3, int m_base, int g_n_iter, int rng_b, int rng_c, int rng_a, int intra_stride,
nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 8, 8, 4, double>&t3_frag)
{
	const int lane = threadIdx.x & 31;
	const int frag_m = lane >> 2;
	...
	int id_m = frag_m - div4;
	int id_n = frag_n + div4;
	if((m_base + id_m) < rng_b && g_n_iter < rng_c && (id_n) < rng_a)
	{
		dev_t3[id_m * intra_stride + id_n] = (div4 ? y : x);
	}
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', 'x', 't2', ['d', 'c', 'a']], [16, 'STR_SD2_V2_H7', '', 'v2', ['b', 'd']], '-']
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				{
					scatter_store_tail2(&dev_t3[dst], m_base, g_n_iter, rng_b, rng_c, rng_a, intra_stride, t3_frag[frag_idx]);
				}
			}
		}
	}
