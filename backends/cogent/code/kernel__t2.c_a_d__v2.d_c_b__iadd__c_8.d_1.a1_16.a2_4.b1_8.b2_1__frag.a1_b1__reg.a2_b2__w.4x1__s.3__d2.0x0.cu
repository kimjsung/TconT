...
#define TILE_UNIT 	(TILE_C * TILE_D)

#define TILE_A 		(TILE_A1 * TILE_A2)

#define TILE_B 		(TILE_B1 * TILE_B2)

#define plane_v2 			(TILE_B1 * TILE_C)
#define v2_elements 		(plane_v2 * TILE_B2)

#define plane_t2 			(TILE_A1 * TILE_C)
#define t2_elements 		(plane_t2 * TILE_A2)
...
int blk_idx_a, int blk_idx_b, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_v2_offset, int shm_t2_offset, int iter_c, int iter_d)
{
	for(int idx = threadIdx.x; idx < v2_elements; idx += blockDim.x)
	{
		int v2_b2 = idx / plane_v2;
		int rem_v2 = idx - v2_b2 * plane_v2;
		int v2_b1 = rem_v2 / TILE_C;
		int v2_c = rem_v2 - v2_b1 * TILE_C;

		int sm_src_v2 = iter_d + (v2_c + iter_c + (blk_idx_b * TILE_B + v2_b2 * TILE_B1 + v2_b1) * size_c) * size_d;

		int sm_dst_v2 = shm_v2_offset + (v2_b2 * (TILE_B1 * TILE_C)) + ((v2_b1 ^ ((v2_c >> 2) << 1)) << 2) + (v2_c & 3) + ((v2_c >> 2) << 5);

		cuda::memcpy_async(thread, &sm_v2[sm_dst_v2], &dev_v2[sm_src_v2], cuda::aligned_size_t<8>{8}, pipeline);
	}

	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		int t2_a2 = idx / plane_t2;
		int rem_t2 = idx - t2_a2 * plane_t2;
		int t2_a1 = rem_t2 / TILE_C;
		int t2_c = rem_t2 - t2_a1 * TILE_C;

		int sm_src_t2 = t2_c + iter_c + (blk_idx_a * TILE_A + t2_a2 * TILE_A1 + t2_a1 + (iter_d) * size_a) * size_c;

		int sm_dst_t2 = shm_t2_offset + (t2_a2 * (TILE_A1 * TILE_C)) + (((t2_a1 ^ ((t2_c >> 2) << 1)) & 7) << 2) + (t2_c & 3) + ((t2_c >> 2) << 5) + ((t2_a1 >> 3) * 64);
...
int blk_idx_a, int blk_idx_b, 
cuda::pipeline<cuda::thread_scope_block> &pipeline, cooperative_groups::thread_block_tile<1U, void> &thread,
int shm_v2_offset, int shm_t2_offset, int iter_c, int iter_d, int internal_upperbound)
{
	for(int idx = threadIdx.x; idx < v2_elements; idx += blockDim.x)
	{
		int v2_b2 = idx / plane_v2;
		...
		if((v2_c < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, &sm_v2[sm_dst_v2], &dev_v2[sm_src_v2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		...
		if((t2_c < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, &sm_t2[sm_dst_t2], &dev_t2[sm_src_t2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
...
int rng_b, int rng_a)
{
	for(int idx = threadIdx.x; idx < v2_elements; idx += blockDim.x)
	{
		...
		if((v2_b2 * TILE_B1 + v2_b1 < rng_b) && (v2_c < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, &sm_v2[sm_dst_v2], &dev_v2[sm_src_v2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
	...
	for(int idx = threadIdx.x; idx < t2_elements; idx += blockDim.x)
	{
		...
		if((t2_a2 * TILE_A1 + t2_a1 < rng_a) && (t2_c < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, &sm_t2[sm_dst_t2], &dev_t2[sm_src_t2], cuda::aligned_size_t<8>{8}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_B1 * TILE_C;
	const int ld_stride_b = TILE_A1 * TILE_C;
	...
	const int v2_fragment_offset = lane;
	const int t2_fragment_offset = lane;
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		{
			int v2_offset = shm_v2_offset + (wrow + iter_v2) * ld_stride_a + (ll << 3);
			v2_frag.x[0] = sm_v2[v2_offset + (v2_fragment_offset ^ (ll << 1))];
			...
			for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
			{
				...
				{
					int t2_offset = shm_t2_offset + (wcol + iter_t2) * ld_stride_b + cnt_t2 * (TILE_C << 3) + (ll << 3);
					const int out_idx = ((iter_v2 * wniter) + iter_t2) * t2_frag_cnt + cnt_t2;
					t2_frag.x[0] = sm_t2[t2_offset + (t2_fragment_offset ^ (ll << 1))];
...
__device__ __forceinline__ void scatter_store_tail(double *dev_t3, int m_base, int n_base, int rng_b, int rng_a, int intra_stride,
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', '', 't2', ['c', 'a', 'd']], [16, 'STR_SD2_V2_H7', '', 'v2', ['d', 'c', 'b']], '+=']
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		{
			int base_iter = base + iter_t2 * inter_n_stride;
			int g_n_iter = (wcol + iter_t2) * TILE_A1;
			#pragma unroll
			for(int cnt_v2 = 0; cnt_v2 < v2_frag_cnt; cnt_v2++)
			{
				...
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++, frag_idx++)
				{
					...
					{
						scatter_store_tail(&dev_t3[dst], m_base, n_base, rng_b, rng_a, intra_stride, t3_frag[frag_idx]);
					}
				}
			}
		}
