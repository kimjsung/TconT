...
int shm_v2_offset, int shm_t2_offset, int iter_e, int iter_f)
{
	for(int idx = threadIdx.x; idx < v2_elements; idx += blockDim.x)
	{
		...
		int sm_src_v2 = iter_f + (blk_idx_d * TILE_D + v2_d + (v2_e + iter_e + (blk_idx_c * TILE_C + v2_c) * size_e) * size_d) * size_f;

		int sm_dst_v2 = shm_v2_offset + (v2_d * ((TILE_C * TILE_E) + 2)) + (v2_c << 2) + (v2_e & 3) + ((v2_e >> 2) << 5);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int sm_src_t2 = t2_e + iter_e + (blk_idx_a * TILE_A + t2_a + (iter_f + (blk_idx_b * TILE_B + t2_b) * size_f) * size_a) * size_e;
...
{
	const int ld_stride_a = TILE_C * TILE_E + 2;
	...
	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		...
		for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
		{
			...
			v2_frag.x[0] = sm_v2[v2_offset + v2_fragment_offset];
			...
			for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
			{
				...
				{
					int t2_offset = shm_t2_offset + ((wcol + iter_t2) * ld_stride_b) + (cnt_t2 << 6) + ((t2_fragment_offset ^ (ll >> 2)) << 2) + t2_lane_offset;
