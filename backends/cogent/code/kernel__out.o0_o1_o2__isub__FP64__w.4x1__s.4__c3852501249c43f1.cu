...
#define plane_v2 			(TILE_D * TILE_B1)
#define v2_elements 		(plane_v2 * TILE_B2)

#define plane_t2 			(TILE_D * TILE_A)
...
int shm_v2_offset, int shm_t2_offset, int iter_d)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		int v2_d = rem_v2 / TILE_B1;
		int v2_b1 = rem_v2 - v2_d * TILE_B1;

		int sm_src_v2 = blk_idx_b * TILE_B + v2_b2 * TILE_B1 + v2_b1 + (v2_d + iter_d) * size_b;

		int sm_dst_v2 = shm_v2_offset + (v2_b2 * (TILE_D * TILE_B1)) + (v2_b1 ^ (((v2_d >> 1) & 1) << 2)) + (v2_d << 3);
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		int t2_d = rem_t2 / TILE_A;
		int t2_a = rem_t2 - t2_d * TILE_A;

		int sm_src_t2 = blk_idx_a * TILE_A + t2_a + (t2_d + iter_d + (blk_idx_c * TILE_C + t2_c) * size_d) * size_a;

		int sm_dst_t2 = shm_t2_offset + (t2_c * (TILE_D * TILE_A)) + (t2_a ^ (((t2_d >> 0) & 3) << 2)) + (t2_d << 4);
...
int rng_b, int rng_c)
{
	for(int idx = (threadIdx.x << 1); idx < v2_elements; idx += (blockDim.x << 1))
	{
		...
		if((v2_b2 * TILE_B1 + v2_b1 < rng_b) && (v2_d < TILE_UNIT - internal_upperbound))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_v2[sm_dst_v2]), reinterpret_cast<const double2*>(&dev_v2[sm_src_v2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		if((t2_d < TILE_UNIT - internal_upperbound) && (t2_c < rng_c))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_t2[sm_dst_t2]), reinterpret_cast<const double2*>(&dev_t2[sm_src_t2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
int rng_c)
{
	...
	for(int idx = (threadIdx.x << 1); idx < t2_elements; idx += (blockDim.x << 1))
	{
		...
		if((t2_a < rng_a) && (t2_d < TILE_UNIT - internal_upperbound) && (t2_c < rng_c))
		{
			cuda::memcpy_async(thread, reinterpret_cast<double2*>(&sm_t2[sm_dst_t2]), reinterpret_cast<const double2*>(&dev_t2[sm_src_t2]), cuda::aligned_size_t<16>{sizeof(double2)}, pipeline);
		}
		else
...
{
	const int ld_stride_a = TILE_D * TILE_B1;
	const int ld_stride_b = TILE_D * TILE_A;
	...
	const int t2_fragment_offset = ((((lane & 3) << 2) ^ ((lane >> 0) & 3)) ^ (lane >> 4));

	const int tmp_v2 = shm_v2_offset + (wrow * ld_stride_a) + (v2_fragment_offset << 2) + v2_lane_offset;
	const int tmp_t2 = shm_t2_offset + (wcol * ld_stride_b) + t2_lane_offset;

	for(int ll = 0; ll < TILE_UNIT; ll += 4)
	{
		const int ll_iter_v2 = ll << 3;
		const int ll_iter_t2 = (ll << 4);
		#pragma unroll
		for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
		{
			const int v2_offset = tmp_v2 + (iter_v2 * ld_stride_a) + ll_iter_v2;
			v2_frag.x[0] = sm_v2[v2_offset];

			#pragma unroll
			for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
			{
				const int tmp_t21 = tmp_t2 + (iter_t2 * ld_stride_b) + ll_iter_t2;
				#pragma unroll
				for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++)
				{
					const int t2_offset = tmp_t21 + ((t2_fragment_offset ^ (cnt_t2 << 1)) << 2);
...
int size_internal, int v2_tile_size, int t2_tile_size)
{
	...
	// Tensor Contraction : [[16, 'STR_SD2_T2_H7', 'x', 't2', ['a', 'd', 'c']], [16, 'STR_SD2_V2_H7', '', 'v2', ['b', 'd']], '-']
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			int base_v2 = base_iter;
			#pragma unroll
			for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++)
			{
				int n_base = (cnt_t2 << 3);
				int dst = base_v2 + (cnt_t2 << 3);
				const int frag_idx = (frag_tmp1 * t2_frag_cnt) + cnt_t2;
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++)
			{
				...
				int dst = base_v2 + (cnt_t2 << 3);
				const int frag_idx = (frag_tmp1 * t2_frag_cnt) + cnt_t2;
				bool full = ((m_base + 8 <= rng_b) && (g_n_iter < rng_c) && (((uintptr_t)(&dev_t3[dst]) & 0x1F) == 0) && ((intra_stride & 1) == 0));
				if(full)
				{
					nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
				}
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++)
			{
				...
				int dst = base_v2 + (cnt_t2 << 3);
				const int frag_idx = (frag_tmp1 * t2_frag_cnt) + cnt_t2;
				bool full = ((m_base + 8 <= rng_b) && (n_base + 8 <= rng_a) && (((uintptr_t)(&dev_t3[dst]) & 0x1F) == 0) && ((intra_stride & 1) == 0));
				if(full)
				{
					nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
				}
	...
	for(int iter_v2 = 0; iter_v2 < wmiter; iter_v2++)
	{
		...
		for(int iter_t2 = 0; iter_t2 < wniter; iter_t2++)
		{
			...
			for(int cnt_t2 = 0; cnt_t2 < t2_frag_cnt; cnt_t2++)
			{
				...
				int dst = base_v2 + (cnt_t2 << 3);
				const int frag_idx = (frag_tmp1 * t2_frag_cnt) + cnt_t2;
				bool full = ((m_base + 8 <= rng_b) && (g_n_iter < rng_c) && (n_base + 8 <= rng_a) && (((uintptr_t)(&dev_t3[dst]) & 0x1F) == 0) && ((intra_stride & 1) == 0));
				if(full)
				{
					nvcuda::wmma::store_matrix_sync(&dev_t3[dst], t3_frag[frag_idx], intra_stride, nvcuda::wmma::mem_row_major);
				}
