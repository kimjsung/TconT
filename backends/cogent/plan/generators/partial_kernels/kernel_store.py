...
def tc_code_kernel_dev_scatter_store_head_fp32(f, kernel_name, ld_tile_order_a, ld_tile_order_b, opt, data_type) :
    ...
    #
    if data_type == "DOUBLE" :
        arguments.append(f"__device__ __forceinline__ void {kernel_name}{opt}(double *__restrict__ dev_t3")
    else :
...
#
def tc_code_kernel_dev_scatter_store_body_fp32(f, ld_tile_order_a, ld_tile_order_b, opt, data_type) :
    #
    frag_partial_a = ld_tile_order_a[1]
    frag_partial_b = ld_tile_order_b[1]
    ...
    if data_type == "DOUBLE" :
        f.write("\tconst bool div = ((((uintptr_t)(&dev_t3[frag_m * intra_stride])) & 0xF) == 0);\n")
    else :
        f.write("\tconst bool div = ((((uintptr_t)(&dev_t3[frag_m * intra_stride])) & 0x7) == 0);\n")

    #
    if data_type == "DOUBLE" :
        f.write(f"\tdouble2 reg = reinterpret_cast<const double2*>(&t3_frag)[0];\n\n")

        #
        if opt == 0 :                           # FRAG_X : full / FRAG_Y : partial
            f.write(f"\tconst bool row = (frag_m < rng_{frag_partial_a});\n")
        #
        elif opt == 1 :                         # FRAG_X : partial / FRAG_Y : full
            f.write(f"\tconst bool col_2 = (frag_n + 1 < rng_{frag_partial_b});\n")
            f.write(f"\tconst bool col_1 = (frag_n < rng_{frag_partial_b});\n")
        #
        ...
        if opt == 0 :
            f.write("\tif(row)\n")
            f.write("\t{\n")
            f.write("\t\tdouble *base = &dev_t3[frag_m * intra_stride];\n")
            f.write("\t\tif(div)\n")
            f.write("\t\t{\n")
            f.write("\t\t\treinterpret_cast<double2*>(&base[frag_n])[0] = reg;\n")
            f.write("\t\t}\n")
            f.write("\t\telse\n")
            f.write("\t\t{\n")
            f.write("\t\t\tbase[frag_n] = reg.x;\n")
            f.write("\t\t\tbase[frag_n + 1] = reg.y;\n")
            f.write("\t\t}\n")
            f.write("\t}\n\n")
        #
        elif opt == 1 :
            f.write("\tdouble *base0 = &dev_t3[frag_m * intra_stride];\n")
            f.write("\tif(col_2 && div)\n")
            f.write("\t{\n")
            f.write("\t\treinterpret_cast<double2*>(&base0[frag_n])[0] = reg;\n")
            f.write("\t}\n")
            f.write("\telse if(col_2)\n")
            f.write("\t{\n")
            f.write("\t\tbase0[frag_n] = reg.x;\n")
            f.write("\t\tbase0[frag_n + 1] = reg.y;\n")
            f.write("\t}\n")
            f.write("\telse if(col_1)\n")
            f.write("\t{\n")
            f.write("\t\tbase0[frag_n] = reg.x;\n")
            f.write("\t}\n\n")
        #
...
def tc_code_kernel_dev_scatter_store(f, kernel_name, fvi_flag, ld_tile_order_a, ld_tile_order_b, collapsed_a, collapsed_b, split_input, data_type) :
    ...
    for i in range(3) :
        ...
        tc_code_kernel_dev_scatter_store_body_fp32(f, ld_tile_order_a, ld_tile_order_b, i, data_type)